...
$query = "SELECT * FROM orders WHERE orderStatus = 'Заказ отменен' AND completionTime >= :currentDate ORDER BY completionTime DESC";
...
if (empty($info_orders)) {
  echo '<p>Ни один заказ еще не отменен</p>';
} else {
$output = '<thead class="fw-bold">';
$output .= '<td>Заказ получен</td>';
$output .= '<td>Заказ отменен</td>';
