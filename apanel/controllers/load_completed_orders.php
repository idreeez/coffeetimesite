...
// Получение данных о выполненных заказах из базы данных
$currentDate = date('Y-m-d');
$query = "SELECT * FROM orders WHERE orderStatus = 'Заказ выполнен' AND completionTime >= :currentDate ORDER BY completionTime DESC";
$stmt = $db->prepare($query);
$stmt->bindParam(':currentDate', $currentDate);
$stmt->execute();
$info_orders = $stmt->fetchAll(PDO::FETCH_ASSOC);

// Генерация HTML-содержимого таблицы выполненных заказов
if (empty($info_orders)) {
  echo '<p>Ни один заказ еще не выполнен</p>';
} else {
$output = '<thead class="fw-bold">';
$output .= '<td>Заказ получен</td>';
$output .= '<td>Заказ выполнен</td>';
...
foreach ($info_orders as $row) {
  ...
  $output .= '<td>' . $row["completionTime"] . '</td>';
  $output .= '<td>' . $row["id"] . '</td>';
...
// Вывод HTML-содержимого таблицы выполненных заказов
echo $output;
?>
